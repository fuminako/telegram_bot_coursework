...
public class NotificationService {
    ...
    public boolean processNotification(Long chatId, String message) {
        ...
        try {
            LocalDateTime notificationDateTime = LocalDateTime.parse(stringDate, DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm"));
    ...
    public void sendNotifications() {
        List<NotificationTask> tasks = this.repository.findByNotificationDateTimeEquals(
                LocalDateTime.now().truncatedTo(ChronoUnit.MINUTES));

        tasks.forEach(notificationTask -> this.telegramBot
                .execute(new SendMessage(notificationTask.getChatId(), notificationTask.getNotificationText())));
        this.repository.deleteAll(tasks);
    }
}
